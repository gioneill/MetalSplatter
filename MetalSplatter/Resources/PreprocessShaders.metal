...
// Keep in sync with SplatRenderer.swift : PreprocessedSplat
...
// Vertex buffer indices for preprocessed path
enum PreprocessedVertexBufferIndex: int32_t {
    PreprocessedVertexBufferIndexUniforms = 0,
    PreprocessedVertexBufferIndexSplat    = 1,
    PreprocessedVertexBufferIndexPreprocessedSplat = 2,
};

float3 calcCovariance2DPreprocess(float3 viewPos,
                        packed_half3 cov3Da,
                        packed_half3 cov3Db,
                        float4x4 viewMatrix,
...
void decomposeCovariancePreprocess(float3 cov2D, thread float2 &v1, thread float2 &v2) {
    float a = cov2D.x;
    float b = cov2D.y;
    float d = cov2D.z;
    float det = a * d - b * b;
...
kernel void splatPreprocessShader(constant UniformsArray& uniformsArray [[ buffer(PreprocessBufferIndexUniforms) ]],
    ...
    float3 cov2D = calcCovariance2DPreprocess(viewPosition, splat.covA, splat.covB,
                                    uniforms.viewMatrix, uniforms.projectionMatrix, uniforms.screenSize);

    float2 axis1;
    float2 axis2;
    decomposeCovariancePreprocess(cov2D, axis1, axis2);
...
vertex FragmentIn preprocessedSplatVertexShader(constant UniformsArray& uniformsArray [[ buffer(PreprocessedVertexBufferIndexUniforms) ]],
                                    constant PreprocessedSplat* preprocessedSplatArray [[ buffer(PreprocessedVertexBufferIndexPreprocessedSplat) ]],
...
fragment half4 preprocessedSplatFragmentShader(FragmentIn in [[stage_in]]) {
