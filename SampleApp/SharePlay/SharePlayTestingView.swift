...
struct SharePlayTestingView: View {
    ...
    private func sendTestMessage() {
        ...
        Task {
            try? await integrationHelper.sessionManager.messenger?.send(.annotation(annotation))
        }
    ...
    private func testModelLoad() {
        ...
        integrationHelper.modelSync.selectModel(.sampleBox)
    }
            ...
            Grid(alignment: .leading, horizontalSpacing: 16, verticalSpacing: 4) {
                ...
                GridRow {
                    ...
                    Text("\(metrics.networkLatencyMs, specifier: "%.0f") ms")
                        .fontFamily(.monospaced)
                        .foregroundColor(metrics.networkLatencyMs > 100 ? .orange : .primary)
                }
        ...
        NavigationView {
            List {
                ...
                Section("Participants") {
                    ForEach(Array(integrationHelper.sessionManager.activeParticipants), id: \.id) { participant in
                ...
                Section("Camera Sync") {
                    ...
                    LabeledContent("Synced Rotation", value: "\(integrationHelper.cameraSync.syncedRotation.degrees, specifier: "%.1f")°")
                }
                
                Section("Performance") {
                    LabeledContent("Memory", value: "\(integrationHelper.performanceMetrics.memoryUsageMB, specifier: "%.1f") MB")
                    LabeledContent("Network Latency", value: "\(integrationHelper.performanceMetrics.networkLatencyMs, specifier: "%.0f") ms")
